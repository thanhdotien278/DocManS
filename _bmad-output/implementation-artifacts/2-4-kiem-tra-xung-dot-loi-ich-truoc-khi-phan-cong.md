...
3. **Deny source participation and reviewer decisions in both directions and at action time.** Given a candidate is an active proposal/topic PI, topic member or topic secretary, when proposed as management officer, reviewer, council evaluator/member/chair/secretary or approval authority on that source record, then the backend returns `CONFLICT_DENIED` / `SOURCE_PARTICIPATION_CONFLICT`. Given the candidate holds an active evaluation position or has a persisted draft/submitted evaluation in the same round, final approval/rejection returns `REVIEWER_DECISION_CONFLICT`; persisted evaluation keeps the conflict after revocation or expiry, while an ended assignment with no evaluation does not. Neither path creates an assignment, access, notification, or decision side effect. Repeat these checks when adding/changing a participant against current management/evaluation duties and at the protected action, not only in preflight; Staff review/participation access never grants management authority.

...
6. **Limit external directory disclosure.** Given the actor is `EXTERNAL_RESEARCHER_USER`, when the directory or profile detail is loaded, then only the minimum fields needed for an explicit related/assigned record are returned; unrelated profiles, contact data, hidden assignments, conflict sources, and privileged system-role data remain unavailable.
7. **Fail protected clients closed.** Given a response has an unknown capability schema/action/decision code or lacks required context, when the web client parses it, then protected actions remain disabled and a refresh/error state is shown. A list/detail capability response is descriptive; every mutation continues to re-authorize.
...
- [ ] Task 4: Verify authorization, query correctness, UX, and performance (AC: 1-8)
  - [ ] Test external-role minimum disclosure, cross-unit and direct-ID denial/no-existence-leak, inactive profiles, empty result, scoped totals/facets, multiple child rows without duplicate profiles, stable page boundaries, invalid filters/page size, and field-level omission.

...
export class ProposalReviewAccessService {
  ...
  async resolveForProposal(userId: string | undefined, proposalId: string): Promise<ProposalReviewAccess> {
    if (!userId || !proposalId) {
      return resolveProposalReviewAccess(null);
    }
  ...
  async resolveForProposals(userId: string | undefined, proposalIds: string[]): Promise<Map<string, ProposalReviewAccess>> {
    ...
    for (const proposalId of proposalIds) {
      resolved.set(proposalId, resolveProposalReviewAccess(byProposal.get(proposalId) ?? []));
    }

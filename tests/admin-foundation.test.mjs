  ...
  return {
    ...
    role: {
      ...
      async findUnique({ where }) {
        return where.code === "system-admin"
          ? { id: "role-admin", code: "system-admin", label: "Quản trị hệ thống", status: "active" }
          : null;
      },
      async create({ data }) {
    ...
    organizationUnit: {
      ...
      async findUnique({ where }) {
        return where.id === "org-root" ? { id: "org-root", code: "HVQY", name: "Học viện Quân y", status: "active" } : null;
      },
      async create({ data }) {
...
describe("admin foundation API behavior", () => {
  ...
  it("creates a user with role and organization scope assignments and an audit row", async () => {
    ...
    assert.equal(auditLog.records[0].action, "create-user");
